...
import auca.com.demo.service.AcademicService;
...
public class AcademicController {
    ...
    @PutMapping("/unit/{id}")
    public ResponseEntity<?> updateUnit(@PathVariable("id") Integer id, @RequestBody AcademicUnit updatedUnit) {
        try {
            AcademicUnit existingUnit = academicService.selectAcademicUnit(id);

            if (existingUnit != null) {
                // Update the properties of the existing unit with the new values
                existingUnit.setCode(updatedUnit.getCode());
                existingUnit.setName(updatedUnit.getName());
                existingUnit.setUnityType(updatedUnit.getUnityType());
                existingUnit.setParentUnit(updatedUnit.getParentUnit());
                // Update any other properties as needed

                // Save the updated unit
                AcademicUnit updatedAcademicUnit = academicService.insertAcademicUnit(existingUnit);

                if (updatedAcademicUnit != null) {
                    return new ResponseEntity<>("AcademicUnit Updated", HttpStatus.OK);
                } else {
                    return new ResponseEntity<>("Update failed", HttpStatus.BAD_REQUEST);
                }
            } else {
                return new ResponseEntity<>("AcademicUnit not found", HttpStatus.NOT_FOUND);
            }
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
